...
foreach my $currentJob (@jobArray) {
  ...
  while (my $moteBasicRef = $moteBasicStatement->fetchrow_hashref()) {
    ...
    if (defined($currentMoteProg[$moteBasicRef->{'moteid'}])) {
      my $sfCommand = "$_EXTERNALSF -comm network\@$moteBasicRef->{'ip_addr'}:"
  ...
  my $tempDir = mkdir($tempDirPath)
    or die "Couldn't create temp directory $tempDirPath\n";

  # zone: find the DBLogger class files
  my $classQuery = "select files.path, files.user, files.type, files.id," .
...
sub doProgram {
  ...
  if ($moteProgram ne "") {
    ...
    #print "$runRoot --upload if=blank.srec 2>&1\n";
    $moteOutput .= `$runRoot --upload if=blank.srec 2>&1`;
  }
  if ($moteProgram ne "") {
