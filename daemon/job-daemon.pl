...
for (my $moteid = 1; $moteid <= @currentMoteProg; $moteid++) {
  ...
  if (defined($topologyId)) { # if topologyId not null, then job uses a topology, and so parse and run the algorithm
    my $dbEdges;
    my $dbNodes;
    ...
    foreach my $node (@nodeStrings) {
      my @nodeSplit = split(",", $node); # split individual node data on ","
