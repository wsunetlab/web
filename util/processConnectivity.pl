...
my $_CONNECTDATATABLENAME = $connectTableRoot ;
my $_CONNECTCONTROLTABLENAME = $connectTableRoot;
...
print "Query is $startEndStatement\n";
...
my $selectGroupNumberQuery = "select groupno from connectivity" .
...
my $clearMoteConnectivityQuery = "update motes set linkquality=\"\"";
...
for (my $i = 1; $i < @heardArray; $i++) {
  ...
  for (my $j = 1; $j < @{$heardArray[$i]}; $j++) {
    if ($heardArray[$i][$j] != undef) {
      my $updateConnectivityQuery = "insert into connectivity" .
  ...
  my $updateMoteConnectivityQuery = "update motes set linkquality=\"" .
                                    $entryString . "\"" .
                                    " where moteid=" . $i;
...
my $lastContactQuery = "update motes set lastcontact = now()" .
                                 " where linkquality <> ''";
